...
#
#   scripts/release.sh prepare 5.3.6   branch chore/release-5.3.6 from master: set the version,
#                                      date the CHANGELOG's Unreleased section, preflight, commit
#   (merge chore/release-5.3.6 into master)
#   scripts/release.sh tag 5.3.6       tag that merge, lightweight like every earlier tag
#
# It never pushes. `git push origin master 5.3.6` is yours; the tag starts
...
echo "$VERSION" | grep -Eq '^[0-9]+\.[0-9]+\.[0-9]+(-[0-9A-Za-z.-]+)?$' || die "'$VERSION' is not a version (5.3.6, 5.4.0-beta.1)"
...
prepare)
  ...
  echo
  echo "chore/release-$VERSION is ready. Merge it into master, then: scripts/release.sh tag $VERSION"
  ;;
tag)
  ...
  ;;
*)
  die "usage: scripts/release.sh prepare|tag <version>"
  ;;
esac
