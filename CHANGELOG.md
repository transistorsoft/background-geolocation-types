...
* [Fixed] `State` no longer declares `reset` or `transistorAuthorizationToken`. Both are `ready()` / `setConfig()` inputs that no SDK reports back: `reset` is a one-shot directive, and the token is expanded into `http.url` and `authorization` before it reaches the native SDK. `State` is still assignable to `Config`, so `setConfig(state)` still compiles. (WO-049)
* [Fixed] `Location.geofence` is a new `GeofenceTrigger` — `{identifier, action, timestamp, extras?}`, the summary every SDK sends — not a `GeofenceEvent`. It never carried a `location` or a `Geofence` record: `location.geofence.location` was always `undefined`, and now fails to compile. `GeofenceTrigger` matches the Swift and Kotlin `LocationEvent.GeofenceTrigger`, and the HTTP schema now shows `geofence.timestamp`. (WO-049)
* [Docs] `NotificationConfig.layout`: your own elements may use any `android:id`. Only the special elements (`applicationName`, `notificationTitle`, `notificationText`, `notificationSmallIcon`, `notificationLargeIcon`) have fixed ids; the documentation said every id needed a `notification` prefix. The last example now fills an element the sample layout has.
